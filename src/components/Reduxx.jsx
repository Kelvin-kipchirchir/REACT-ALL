...
class Reduxx extends React.Component {
     constructor(props) { 
         super(props); 
...
handleDelete = (id,e) => {
    e.preventDefault(); this.props.onDelete(id); 
} 
getTotal() { 
    let total = 0; 
...
render() { 
    ...
    <tr key={item.id}> <td>{item.name}</td> 
        ...
        <td>{item.category}</td> <td>
            <a href="#" onClick={(e) => this.handleDelete(item.id, e)}>Remove</a></td> 
    </tr> ); 
    return ( 
    <div> 
        <table> 
            ...
            <tr> 
               ...
               <td colSpan="4" style={{ textAlign: "left" }}> {this.getTotal()} </td> 
            </tr> 
            </tbody> 
        </table> 
        </div> ); 
